...
const MODELES = {
  ...
  /* --- Évaluation --- */
  /* ⚠️ LES DEUX ÉVALUATIONS SE REMPLISSENT À LA MAIN — v924.

     David : « on avait pas dit qu'on enlevait le bouton de vocal
     pour les évaluations et qu'on forçait à le faire en manuel avec
     toutes les catégories à remplir à la main ? »

     C'est un barème, pas un récit : on note des cases, on ne
     raconte pas la leçon. Dicter un tableau de notes n'a pas de
     sens, et une évaluation dictée revient à côté du barème qu'elle
     est censée remplir. */
  'eval-manuelle': {
    label: 'Évaluation — Boîte manuelle', groupe: 'Évaluation', schema: 'eval',
    build: ai => buildEval(ai, true), manuelSeul: true
  },
  'eval-auto': {
  ...
  'examen-blanc': {
    ...
    build: buildExamenBlanc, manuelSeul: true
  },
  'examen-officiel': {
    label: 'Examen officiel', groupe: 'Examen', schema: 'examen',
    build: buildExamen, manuelSeul: true
  },
