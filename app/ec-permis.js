...
/* Nom propre à ce module : ec-messenger.js déclarait une fonction
   du même nom, chargée après, qui écrasait celle-ci. Les deux ne
   produisent pas le même bloc. */
function blocCopiablePermis(titre, texte){
...
function afficherPermis(nom, dossier){
  ...
  function rendre(){
    ...
    contenu.appendChild(blocCopiablePermis('1 · Félicitations', MSG_PERMIS.felicitations));
    contenu.appendChild(blocCopiablePermis(
      '2 · Demande ANTS — ' + (ants === 'nous' ? 'on s\'en occupe' : 's\'en occupe lui-même'),
      ants === 'nous' ? MSG_PERMIS.antsNous : MSG_PERMIS.antsEleve));
    if(boite === 'bea'){
      contenu.appendChild(blocCopiablePermis('3 · Passerelle BEA → BV', MSG_PERMIS.passerelle));
    }
