...
async function chargerResultatsConnus(){
  ...
  try{
    ...
    ((r && r.resultats) || []).forEach(x => {
      ...
      if(!k) return;
      const iso = String(x.iso || '') || dateFrVersIso(x.date || '');

      /* ⚠️ ON GARDE TOUTES LES DATES, PAS SEULEMENT LA DERNIÈRE
         LIGNE RENCONTRÉE — v987.

         Cette table sert à répondre « son résultat est-il
         consigné ? », et elle ne retenait qu'UN résultat par
         personne. La Perle en avait un, celui du 24 août : son
         examen du 14 septembre était donc déclaré réglé sans que
         personne ne l'ait noté, et elle n'est jamais revenue dans
         la liste. Un élève qui repasse a plusieurs examens ; il lui
         faut plusieurs dates. */
      const e = parNom[k] || (parNom[k] = { isos: [] });
      if(iso && e.isos.indexOf(iso) === -1) e.isos.push(iso);

      /* Le PLUS RÉCENT fait foi pour le reste : « a-t-il eu son
         permis ? » se lit sur son dernier examen, pas sur celui que
         la feuille mentionne en premier. */
      if(!e.iso || (iso && iso >= e.iso)){
        Object.assign(e, x, { isos: e.isos, iso: iso });
      }
    });
  }catch(e){
...
/* ============================================================
   ⚠️ « UN RÉSULTAT EXISTE POUR CETTE PERSONNE » N'EST PAS
      « CET EXAMEN-CI EST NOTÉ » — v987

   David, le 15 septembre : « J'ai toujours pas la perle dans
   résultat permis et j'ai ceux que j'ai mis obtenu qui reviennent
   ici tout seuls ».

   Deux portes posaient la question, et toutes les deux la posaient
   de travers — c'est encore le même fait lu comme un autre :

     · la liste des places passées écartait qui avait UN résultat,
       quelle qu'en soit la date : La Perle, notée le 24 août, ne
       pouvait plus jamais revenir pour un examen suivant ;

     · et le filtre de l'écran, en v985, se rabattait sur la date
       d'ajournement de la fiche. Un élève qui a OBTENU son permis
       n'a pas de date d'ajournement : le filtre montrait tout le
       monde, et ceux qu'on venait de sortir revenaient seuls.

   La bonne question porte sur un COUPLE — cet élève, cet examen-là
   — et la seule table qui le sait est la feuille des Résultats,
   qui enregistre bien (élève, date d'examen, résultat). Les deux
   portes lisent désormais la même, par ici.
   ============================================================ */
function resultatConsignePour(parNom, nom, iso){
  const e = (parNom || {})[normaliserMot(nom || '')];
  if(!e) return false;

  const d = dateFrVersIso(iso || '') || String(iso || '');
  /* Sans date d'examen, on ne peut pas trancher : on s'en tient à
     l'ancienne règle, « il a un résultat quelque part ». */
  if(!d) return true;

  return (e.isos || []).indexOf(d) !== -1;
}
    ...
    (typeof sessionsPermis !== 'undefined' ? (sessionsPermis || []) : []).forEach(se => {
      ...
      (se.eleves || []).forEach(p => {
        ...
        /* Le résultat DE CET EXAMEN-LÀ est consigné : la question
           ...
           resultatConsignePour. */
        if(resultatConsignePour(dejaConsigne, p.eleve, se.date)) return;
