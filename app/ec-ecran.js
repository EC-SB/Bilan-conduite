...
function blocAdresses(){
  ...
  a.innerHTML = 'À ouvrir sur le téléviseur, en plein écran. Remplace ' +
    '<strong>LE_JETON</strong> par celui réglé dans Cloudflare ' +
    '(<em>ECRAN_TOKEN</em>).<br>Ces adresses ne donnent accès qu\'à l\'affichage : ' +
    'ni bilans, ni élèves, ni réglages.';
  d.appendChild(a);

  const base = 'https://ec-sb.github.io/Bilan-conduite/ecran.html';
  [['🏠 Accueil — avec le planning', base + '?t=LE_JETON&ou=accueil&anonyme=1'],
   ['🪟 Vitrine — sans le planning', base + '?t=LE_JETON&ou=vitrine']]
  .forEach(([nom, url]) => {
    ...
    z.value = url;
    z.readOnly = true;
    z.style.cssText = 'font-size:12px;padding:9px 10px;margin:0 0 6px;';
  ...
  bOk.addEventListener('click', async () => {
    ...
    try{
      await appelPrep({
        action: 'ecranSet',
        id: d ? d.id : '',
        type: imageChoisie ? 'image' : 'message',
        titre: titre,
        contenu: texte,
